...
class DigitClassificationModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=16, kernel_size=(3, 3), padding=0, stride=1, bias=False)
        self.bn1 = nn.BatchNorm2d(16)
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=(3, 3), padding=0, stride=1, bias=False)
        self.bn2 = nn.BatchNorm2d(32)
        self.conv3 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(3, 3), padding=0, stride=1, bias=False)
    ...
    def train_model(self, data_train, data_val=None):
        ...
        for epoch in range(150):
            for batch_idx, (data, target) in enumerate(loader_train):
                ...
                accuracy = torch.mean((torch.max(output, dim=1)[1] == target).float())
                if batch_idx % 4 == 0:
                    print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.4f}\tAccuracy:0%'.format(
                        epoch, batch_idx, len(loader_train),
...
class DigitAttackModel(object):
    ...
    def attack(self, x, target, epsilon):
        ...
        loss = creiteron(output,target)
        loss.backward()
        x_adv = x + torch.sign(x.grad.data)*epsilon
        # x_adv = None
...
class LanguageIDModel(nn.Module):
    ...
    def __init__(self):
        ...
        # Initialize your model parameters here
        "*** YOUR CODE HERE ***"
        self.rnn = nn.RNN(47,20,3)
        self.linn = nn.Linear(20,5)
...
class DeepQModel(nn.Module):
    ...
    def forward(self, states, Q_target=None):
        ...
        if Q_target is not None:
            ...
            loss = creiteron(output,Q_target)
            return loss
        else:
            "*** YOUR CODE HERE ***"
